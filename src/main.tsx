...
type Assessment = { outcome:"needs_information"|"ready_for_human_review"; missingRequirements:string[]; recommendedStep:string; rationale:string[] };
...
function App() {
  ...
  async function submit() {
    ...
    try {
      const create = await fetch("/api/applicants", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify(profile) });
      if (!create.ok) throw new Error("Please check your name and email address.");
      const {id} = await create.json();
      if (files.length) { const form = new FormData(); files.forEach(file => form.append("documents", file)); await fetch(`/api/applicants/${id}/documents`, {method:"POST", body:form}); }
      const response = await fetch(`/api/applicants/${id}/assess`, {method:"POST"});
      if (!response.ok) throw new Error("Assessment could not be completed.");
      setAssessment(await response.json()); setSubmitted(true);
    } catch (error) { setApiError(error instanceof Error ? error.message : "Could not contact the API. Start the backend first."); }
  ...
  return <main>
    ...
    <section className="grid"><form onSubmit={e => { e.preventDefault(); void submit(); }}>
      ...
      {apiError && <p className="error">{apiError}</p>}<button type="submit" disabled={loading}>{loading ? "Analysing…" : "Analyse my journey"}</button>
    </form>
    <aside><h2>Journey review</h2>{submitted && assessment ? <><div className={assessment.outcome === "ready_for_human_review" ? "result good" : "result"}><b>{assessment.outcome === "ready_for_human_review" ? "Ready for human review" : "More information needed"}</b><p>{assessment.recommendedStep}</p></div><h3>{assessment.missingRequirements.length ? "Please provide" : "Review notes"}</h3><ul>{(assessment.missingRequirements.length ? assessment.missingRequirements : assessment.rationale).map(item => <li key={item}>{item}</li>)}</ul></> : <><div className="result"><b>Your private profile workspace</b><p>Complete the form and the assistant will identify gaps and suggest your appropriate journey.</p></div><h3>What happens next</h3><ol><li>We structure your information</li><li>We check requirements</li><li>You receive a recommended action</li></ol></>}</aside>
